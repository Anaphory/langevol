...
/**
 * Matrix represented by flat arrays -- otherwise the same as default Eigen decomposition system of BEAST
 *
 * @author Andrew Rambaut
 * @author Marc Suchard
 * @author Ziheng Yang?
 */
public class FlatMatrixEigenSystem implements EigenSystem {

    private final int stateCount;

    public FlatMatrixEigenSystem(int stateCount) {
    ...
    private void elmhes(double[] a, int[] ordr, int n) {
        ...
        for (m = 2; m < n; m++) {
            ...
            i = m;
            for (j = m; j <= n; j++) {
                if (Math.abs(a[(j - 1)*n+m - 2]) > Math.abs(x)) {
                    x = a[(j - 1)*n+m - 2];
                    i = j;
                }
            }
    ...
    private void hqr2(int n, int low, int hgh, double[] h, double[] zz,
        ...
        while (en >= low) {    /* search for next eigenvalues */
            ...
            while (en >= 1) {
                ...
                for (l = en; l > low; l--) {
                    s = Math.abs(h[(l - 2)*n+l - 2]) + Math.abs(h[(l - 1)*n+l - 1]);
                    if (s == 0.0) {
                        s = norm;
                    }
                    tst1 = s;
                    tst2 = tst1 + Math.abs(h[(l - 1)*n+l - 2]);
                    if (tst2 == tst1) {
                        fullLoop = false;
                        break;
                    }
                ...
                if (its == 10 || its == 20) {
                    ...
                    s = Math.abs(h[(en - 1)*n+na - 1]) + Math.abs(h[(na - 1)*n+en - 3]);
                    x = 0.75 * s;
                    y = x;
                    w = -0.4375 * s * s;
                }
                ...
                for (m = en - 2; m >= l; m--) {
                    ...
                    tst1 = Math.abs(p) * (Math.abs(h[(m - 2)*n+m - 2]) + Math.abs(z) + Math.abs(h[(m)*n+m]));
                    tst2 = tst1 + Math.abs(h[(m - 1)*n+m - 2]) * (Math.abs(q) + Math.abs(r));
                    if (tst2 == tst1) {
                        break;
                    }
                }
                ...
                for (k = m; k <= na; k++) {
                    ...
                    if (x != 0.0) {
                        ...
                        if (!notLast) {
                            ...
                            for (i = low - 1; i < hgh; i++) {
                                p = x * zz[(i)*n+k - 1] + y * zz[(i)*n+k];
                                zz[(i)*n+k - 1] -= p;
                                zz[(i)*n+k] -= p * q;
                            }
                        } else {
                            for (j = k - 1; j < n; j++) {    /* row modification */
                                p = h[(k - 1)*n+j] + q * h[(k)*n+j] + r * h[(k + 1)*n+j];
                                h[(k - 1)*n+j] -= p * x;
                                h[(k)*n+j] -= p * y;
                                h[(k + 1)*n+j] -= p * z;
                            }
                            j = (en < (k + 3)) ? en : (k + 3); /* min */
                            for (i = 0; i < j; i++) {    /* column modification */
                                p = x * h[(i)*n+k - 1] + y * h[(i)*n+k] + z * h[(i)*n+k + 1];
                                h[(i)*n+k - 1] -= p;
                                h[(i)*n+k] -= p * q;
                                h[(i)*n+k + 1] -= p * r;
                            }
            ...
            if (q >= 0.0) {     /* real pair */
                ...
                for (i = 0; i < en; i++) {    /* column modification */
                    z = h[(i)*n+na - 1];
                    h[(i)*n+na - 1] = q * z + p * h[(i)*n+en - 1];
                    h[(i)*n+en - 1] = q * h[(i)*n+en - 1] - p * z;
                }
                /* accumulate transformations */
                for (i = low - 1; i < hgh; i++) {
                    z = zz[(i)*n+na - 1];
                    zz[(i)*n+na - 1] = q * z + p * zz[(i)*n+en - 1];
                    zz[(i)*n+en - 1] = q * zz[(i)*n+en - 1] - p * z;
                }
            } else {    /* complex pair */
                wr[na - 1] = x + p;
                wr[en - 1] = x + p;
        ...
        if (norm != 0.0) {
            for (en = n; en >= 1; en--) {
                ...
                if (q == 0.0) {/* real vector */
                            ...
                            if (wi[i] < 0.0) {
                                ...
                                if (wi[i] == 0.0) {
                                    ...
                                    if (Math.abs(x) > Math.abs(z))
                                        h[(i + 1)*n+en - 1] = (-r - w * t) / x;
                                    else
                                        h[(i + 1)*n+en - 1] = (-s - y * t) / z;
                                }
                                /* overflow control */
                                t = Math.abs(h[(i)*n+en - 1]);
                                if (t != 0.0) {
                                    tst1 = t;
                                    tst2 = tst1 + 1.0 / tst1;
                                    if (tst2 <= tst1) {
                    ...
                    m = na;
                    if (Math.abs(h[(en - 1)*n+na - 1]) > Math.abs(h[(na - 1)*n+en - 1])) {
                        h[(na - 1)*n+na - 1] = q / h[(en - 1)*n+na - 1];
                        h[(na - 1)*n+en - 1] = (p - h[(en - 1)*n+en - 1]) / h[(en - 1)*n+na - 1];
                    } else {
                        mcdiv(0.0, -h[(na - 1)*n+en - 1], h[(na - 1)*n+na - 1] - p, q);
                        h[(na - 1)*n+na - 1] = cr;
                        h[(na - 1)*n+en - 1] = ci;
                    }
                    h[(en - 1)*n+na - 1] = 0.0;
                    ...
                    if (en != 2) {
                        for (i = en - 3; i >= 0; i--) {
                            ...
                            sa = 0.0;
                            for (j = m - 1; j < en; j++) {
                                ra += h[(i)*n+j] * h[(j)*n+na - 1];
                                sa += h[(i)*n+j] * h[(j)*n+en - 1];
                            }
                            if (wi[i] < 0.0) {
                                z = w;
                                r = ra;
                                ...
                                if (wi[i] == 0.0) {
                                    ...
                                    if (Math.abs(x) > Math.abs(z) + Math.abs(q)) {
                                        h[(i + 1)*n+
                                                na - 1] = (q * h[(i)*n+en - 1] -
                                                w * h[(i)*n+na - 1] - ra) / x;
                                        h[(i + 1)*n+en - 1] = (-sa - w * h[(i)*n+en - 1] -
                                                q * h[(i)*n+na - 1]) / x;
                                    } else {
                                        mcdiv(-r - y * h[(i)*n+na - 1], -s - y * h[(i)*n+en - 1], z, q);
                                        h[(i + 1)*n+na - 1] = cr;
                                        h[(i + 1)*n+en - 1] = ci;
                                    }
                                }
                                /* overflow control */
                                t = (Math.abs(h[(i)*n+na - 1]) > Math.abs(h[(i)*n+en - 1])) ?
                                        Math.abs(h[(i)*n+na - 1]) : Math.abs(h[(i)*n+en - 1]);
                                if (t != 0.0) {
                                    tst1 = t;
                                    tst2 = tst1 + 1.0 / tst1;
                                    if (tst2 <= tst1) {
    ...
    private void eltran(double[] a, double[] zz, int[] ordr, int n) {
        ...
        for (m = n - 1; m >= 2; m--) {
            for (i = m; i < n; i++) {
                zz[i*n+m - 1] = a[i*n+m - 2];
            }
            i = ordr[m - 1];
            if (i != m) {
                for (j = m - 1; j < n; j++) {
                    zz[(m - 1)*n+j] = zz[(i - 1)*n+j];
                    zz[(i - 1)*n+j] = 0.0;
                }
                zz[(i - 1)*n+m - 1] = 1.0;
            }
    ...
    private void luinverse(double[] inmat, double[] imtrx, int n) throws IllegalArgumentException {
        ...
        index = new int[n];
        omtrx = new double[(n)*n+n];

        /* copy inmat to omtrx */
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                omtrx[(i)*n+j] = inmat[i*n+j];
            }
        }
        ...
        for (i = 0; i < n; i++) {
            maxb = 0.0;
            for (j = 0; j < n; j++) {
                if (Math.abs(omtrx[(i)*n+j]) > maxb) {
                    maxb = Math.abs(omtrx[(i)*n+j]);
                }
            }
            if (maxb == 0.0) {
                /* Singular matrix */
        ...
        for (j = 0; j < n; j++) {
            ...
            index[j] = maxi;
            if (omtrx[(j)*n+j] == 0.0) {
                omtrx[(j)*n+j] = MachineAccuracy.EPSILON;
            }
            if (j != n - 1) {
                tmp = 1.0 / omtrx[(j)*n+j];
                for (i = j + 1; i < n; i++) {
